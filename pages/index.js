...
export default function MickiDashboard() {
  ...
  const [chatLog, setChatLog] = useState([
    { role: 'ai', text: 'System V3.0 Online. Jeg er klar til at modtage informationer, MICKI.' }
  ]);

  const handleSend = () => {
    if (!inputText.trim()) return;
    
    // Tilføjer brugerens besked til loggen
    const newLog = [...chatLog, { role: 'user', text: inputText }];
    setChatLog(newLog);
    
    // AI svarer direkte
    setTimeout(() => {
      setChatLog([...newLog, { role: 'ai', text: `Information modtaget og arkiveret: "${inputText}". Fortsæt med at fodre systemet.` }]);
    }, 500);
  ...
  return (
    <div style={{ backgroundColor: '#050214', color: 'white', minHeight: '100vh', padding: '20px', fontFamily: '-apple-system, sans-serif', position: 'relative', overflowX: 'hidden' }}>
      ...
      {/* NEON KANTER & VANDMÆRKE */}
      <div style={{ position: 'fixed', inset: 0, background: 'radial-gradient(circle at top left, rgba(255,49,49,0.3) 0%, transparent 50%), radial-gradient(circle at bottom right, rgba(0,210,255,0.3) 0%, transparent 50%)', zIndex: 1, pointerEvents: 'none' }} />
      <div style={{ position: 'fixed', inset: 0, opacity: 0.08, backgroundImage: 'url(/M_logo.png)', backgroundSize: '90%', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', zIndex: 1, pointerEvents: 'none' }} />
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: '500px', margin: '0 auto', paddingBottom: '140px' }}>
        ...
        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '30px', borderBottom: '2px solid #ff3131', paddingBottom: '15px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
            ...
            <h1 style={{ fontSize: '22px', fontWeight: '900', letterSpacing: '1px' }}>MICKI_CORE</h1>
          </div>
          <div style={{ fontSize: '10px', color: '#00d2ff', fontWeight: 'bold', border: '1px solid #00d2ff', padding: '4px 8px', borderRadius: '5px' }}>LEARNING_MODE: ON</div>
        </header>

        {/* NYHEDER (Større overskrifter + Liste) */}
        <section style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(25px)', padding: '25px', borderRadius: '35px', border: '1px solid rgba(255,49,49,0.3)', marginBottom: '20px' }}>
        ...
        {/* KALENDER */}
        <section style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(25px)', padding: '25px', borderRadius: '35px', border: '1px solid rgba(0,210,255,0.3)', marginBottom: '20px' }}>
        ...
        {/* CHAT LOG (Svarer når du skriver) */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '0 10px', marginBottom: '20px' }}>
          {chatLog.slice(-3).map((msg, i) => (
            <div key={i} style={{ alignSelf: msg.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '85%' }}>
              <div style={{ padding: '15px 20px', borderRadius: '22px', fontSize: '12px', fontWeight: 'bold', background: msg.role === 'user' ? 'rgba(255,49,49,0.2)' : 'rgba(0,210,255,0.15)', border: '1px solid rgba(255,255,255,0.1)', boxShadow: msg.role === 'user' ? 'none' : '0 0 15px rgba(0,210,255,0.1)' }}>
                {msg.text}
              </div>
            </div>
          ))}
        ...
        <div style={{ position: 'fixed', bottom: '30px', left: '20px', right: '20px', maxWidth: '500px', margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: '12px' }}>
            <input 
              ...
              style={{ flex: 1, padding: '22px 30px', background: 'rgba(5, 2, 20, 0.95)', border: '2px solid #ff3131', borderRadius: '50px', color: 'white', fontSize: '14px', fontWeight: 'bold', outline: 'none', boxShadow: '0 0 30px rgba(255,49,49,0.3)', backdropFilter: 'blur(15px)' }}
            />
            <button onClick={handleSend} style={{ width: '65px', height: '65px', background: '#ff3131', borderRadius: '50%', border: 'none', color: 'white', fontWeight: 'black', fontSize: '24px', cursor: 'pointer', boxShadow: '0 0 20px rgba(255,49,49,0.5)' }}>↑</button>
          </div>
        </div>
